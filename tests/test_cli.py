...
def test_experience_accept_locks_draft_to_career_profile(
    monkeypatch,
    tmp_path,
) -> None:
    import career_agent.cli as cli_module
    ...
    assert result.exit_code == 0
    assert "Locked draft experience entry" in result.output
    assert session is not None
    assert session.status.value == "locked"
...
def test_experience_lock_saves_draft_to_career_profile(
    monkeypatch,
    tmp_path,
) -> None:
    import career_agent.cli as cli_module
    ...
    result = runner.invoke(app, ["experience", "lock", session_id])
    session = intake_repository.load_session(session_id)
